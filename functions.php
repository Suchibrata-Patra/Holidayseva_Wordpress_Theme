...
// Add custom rewrite rule for tours search
function custom_search_rewrite_rules() {
    add_rewrite_rule(
        '^tours/search/?$', // Custom URL structure
        'index.php?post_type=book', // Map to query vars
        'top'
    );

    // Add additional rules for filter parameters if needed
    add_rewrite_rule(
        '^tours/search/([^/]+)/([^/]+)/([^/]+)/?$',
        'index.php?post_type=book&price_filter=$matches[1]&author_filter=$matches[2]&language_filter=$matches[3]',
        'top'
    );
}
...
function custom_query_vars($vars) {
    $vars[] = 'price_filter';
    $vars[] = 'author_filter';
    $vars[] = 'language_filter';
    return $vars;
}
add_filter('query_vars', 'custom_query_vars');

// Handle search query in pre_get_posts
function custom_pre_get_posts($query) {
    if (!is_admin() && $query->is_main_query() && $query->get('post_type') === 'book') {
        // Apply filters based on query vars
        $meta_query = array('relation' => 'AND');
        if ($price_filter = get_query_var('price_filter')) {
            $meta_query[] = array(
                'key' => '_book_price',
                'value' => $price_filter,
                'compare' => '<='
        ...
        if ($author_filter = get_query_var('author_filter')) {
            $query->set('author_name', $author_filter);
        }
        if ($language_filter = get_query_var('language_filter')) {
            $meta_query[] = array(
                'key' => '_book_language',
