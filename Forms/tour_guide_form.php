...
<div class="tg-meta-container">
    ...
    <div style="margin-bottom: 25px;">
        <input type="hidden" name="tg_featured_image" id="tg_featured_image"
            value="<?php echo esc_attr($meta['featured_image']); ?>">
        <div class="tg-image-wrapper">
            <?php if ($meta['featured_image']) : ?>
            <img id="tg_featured_preview" src="<?php echo esc_url(wp_get_attachment_url($meta['featured_image'])); ?>"
                style="max-width: 40%; height: auto;">
            <?php else : ?>
            <p><em>No featured image selected yet.</em></p>
            <img id="tg_featured_preview" style="max-width: 40%; height: auto; display: none;">
            <?php endif; ?>
            <button type="button" class="button upload_image_button" data-target="featured">Upload Featured
                Image</button>
            <button type="button" class="button remove_image_button" data-target="featured"
    ...
    <div class="tg-main">
        ...
        <p>
            <label for="tg_<?php echo $field; ?>"><strong>
            ...
            <?php else: ?>
            <textarea name="tg_<?php echo $field; ?>" id="tg_<?php echo $field; ?>" class="widefat"
                rows="3"><?php echo esc_textarea($meta[$field]); ?></textarea>
            <?php endif; ?>
        </p>
        <?php endforeach; ?>

        <hr>
        <h2>Section-wise Image Uploads</h2>
        <?php foreach ($fields as $field): ?>
        <div style="margin-bottom: 25px;">
            <p><strong>
                    <?php echo ucwords(str_replace('_', ' ', $field)); ?> Image:
                </strong></p>
            <input type="hidden" name="tg_<?php echo $field; ?>_image" id="tg_<?php echo $field; ?>_image"
                value="<?php echo esc_attr($meta[" {$field}_image"]); ?>">

            <div class="tg-image-wrapper"
                style="display: flex; align-items: center; justify-content: start; gap: 30px;">
                <div>
                    <?php if ($meta["{$field}_image"]) : ?>
                    <img id="tg_<?php echo $field; ?>_preview" src="<?php echo esc_url(wp_get_attachment_url($meta["
                        {$field}_image"])); ?>"
                    style="max-width: 150px; height: auto;">
                    <?php else : ?>
                    <p><em>No image selected yet.</em></p>
                    <img id="tg_<?php echo $field; ?>_preview" style="max-width: 150px; height: auto; display: none;">
                    <?php endif; ?>
                </div>

                <div style="display: flex; flex-direction: column; gap: 8px;">
                    <button type="button" class="button upload_image_button" data-target="<?php echo $field; ?>">
                        Upload Image
                    </button>
                    <button type="button" class="button remove_image_button" data-target="<?php echo $field; ?>">
                        Remove
                    </button>
                </div>
            </div>
